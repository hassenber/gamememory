...
return string;}
function forEach(object,callback){var name,i=0,value,length=object?object.length:UNDEFINED,isObj=length===UNDEFINED;if(isObj){for(name in object){if(object.hasOwnProperty(name)){if(callback.call(object[name],name,object[name])===FALSE){break;}}}}else{for(value=object[0];i<length&&callback.call(value,i,value)!==FALSE;value=object[++i]){continue;}}}
function determineSelectorMethod(){var engines=ENGINES,method;forEach(engines,function(engine,obj){if(win[engine]&&!method){method=eval(obj.s.replace("*",engine));if(method){ENGINE=engine;return false;}}});return method;}
...
var engines=ENGINES,method,caller,args;forEach(engines,function(engine,obj){if(win[engine]&&!method&&obj.m){method=eval(obj.m.replace("*",engine));caller=obj.c?eval(obj.c.replace("*",engine)):win;args=[];if(method&&caller){if(obj.p){args.push(obj.p);}
args.push(handler);method.apply(caller,args);return false;}}});if(!method){addEvent("load",handler);}}
...
break;case"orient":case"align":case"direction":case"pack":addRules(selector,rule,NULL,NULL);break;case"flex":case"flex-group":case"ordinal-group":addRules(selector,rule,shortProp,value);break;}}});});});forEach(uniqueBoxes,function(key,box){FLEX_BOXES.push(box);});forEach(uniqueChildren,function(key,child){POSSIBLE_FLEX_CHILDREN.push(child);});return{boxes:FLEX_BOXES,children:POSSIBLE_FLEX_CHILDREN};}
function matchFlexChildren(parent,lib,possibleChildren){var caller,unique,matches=[];forEach(possibleChildren,function(i,child){if(child.selector){caller=lib(child.selector);caller=caller[0]?caller:[caller];if(caller[0]){forEach(caller,function(i,node){if(node.nodeName!==UNDEFINED){switch(node.nodeName.toLowerCase()){case"script":case"style":case"link":break;default:if(node.parentNode===parent){setFlexieId(node);unique={};forEach(child,function(key){unique[key]=child[key];});unique.match=node;matches.push(unique);}
break;}}});}}else{setFlexieId(child);matches.push({match:child,selector:buildSelector(child)});}});return matches;}
function getParams(params){forEach(params,function(key,value){params[key]=value||DEFAULTS[key];});return params;}
function buildFlexieCall(flexers){var selector,properties,property,value,shortProp,display,orient,align,direction,pack,lib,caller,children,box,params,flexboxes={},match,childMatch,nestedFlexboxes,flexieParentSelector="["+FLX_PARENT_ATTR+"]";if(!flexers){return;}
forEach(flexers.boxes,function(i,flex){flex.selector=trim(flex.selector);selector=flex.selector;properties=flex.properties;display=orient=align=direction=pack=NULL;forEach(properties,function(i,prop){property=trim(prop.property);value=trim(prop.value);if(property){shortProp=property.replace("box-",EMPTY_STRING);switch(shortProp){case"display":if(value==="box"){display=value;}
break;case"orient":orient=value;break;case"align":align=value;break;case"direction":direction=value;break;case"pack":pack=value;break;}}});lib=LIBRARY;caller=lib(flex.selector);caller=caller[0]?caller:[caller];forEach(caller,function(i,target){if(target.nodeType){setFlexieId(target);children=matchFlexChildren(target,lib,flexers.children);nestedFlexboxes=selector+" "+flexieParentSelector;params={target:target,selector:selector,properties:properties,children:children,display:display,orient:orient,align:align,direction:direction,pack:pack,nested:nestedFlexboxes};match=flexboxes[target.FLX_DOM_ID];if(match){forEach(params,function(key,value){switch(key){case"selector":if(value&&!(new RegExp(value).test(match[key]))){match[key]+=", "+value;}
break;case"children":forEach(params[key],function(k,child){childMatch=FALSE;forEach(match[key],function(key,existing){if(child.match.FLX_DOM_ID===existing.match.FLX_DOM_ID){childMatch=TRUE;}});if(!childMatch){match[key].push(child);}});break;default:if(value){match[key]=value;}
break;}});}else{flexboxes[target.FLX_DOM_ID]=getParams(params);flexboxes[target.FLX_DOM_ID].target.setAttribute(FLX_PARENT_ATTR,TRUE);}}});});DOM_ORDERED=LIBRARY(flexieParentSelector);FLEX_BOXES={};forEach(DOM_ORDERED,function(i,target){FLEX_BOXES[target.FLX_DOM_ID]=flexboxes[target.FLX_DOM_ID];});forEach(FLEX_BOXES,function(key,flex){if(flex.display==="box"){box=new FLX.box(flex);}});}
function calcPx(element,props,dir){var dim=dir.replace(dir.charAt(0),dir.charAt(0).toUpperCase()),value=element["offset"+dim]||0;if(value){forEach(props,function(i,prop){prop=parseFloat(element.currentStyle[prop]);if(!isNaN(prop)){value-=prop;}});}
return value;}
...
function appendProperty(target,prop,value,prefixName){var cssText=[];forEach(PREFIXES,function(i,prefix){cssText.push((prefixName?prefix:EMPTY_STRING)+prop+":"+(!prefixName?prefix:EMPTY_STRING)+value);});target.style.cssText+=cssText.join(";");return target;}
function appendPixelValue(target,prop,value){var targets=target&&target[0]?target:[target];forEach(targets,function(i,target){if(target&&target.style){target.style[prop]=(value?(value+"px"):EMPTY_STRING);}});}
function calculateSpecificity(selector){var selectorGrid,matrix,total;selectorGrid=selector.replace(CSS_SELECTOR,function(e,f){return"%"+f;}).replace(/\s|\>|\+|\~/g,"%").split(/%/g);matrix={_id:100,_class:10,_tag:1};total=0;forEach(selectorGrid,function(i,chunk){if((/#/).test(chunk)){total+=matrix._id;}else if((/\.|\[|\:/).test(chunk)){total+=matrix._class;}else if((/[a-zA-Z]+/).test(chunk)){total+=matrix._tag;}});return total;}
function filterDuplicates(matches,children,type){var filteredMatches=[],exists,spec=(type?"ordinal":"flex")+"Specificity";forEach(matches,function(i,x){if((!type&&x.flex)||(type&&x["ordinal-group"])){x[spec]=x[spec]||calculateSpecificity(x.selector);exists=FALSE;forEach(filteredMatches,function(j,f){if(f.match===x.match){if(f[spec]<x[spec]){filteredMatches[j]=x;}
exists=TRUE;return FALSE;}});if(!exists){filteredMatches.push(x);}}});return filteredMatches;}
function createMatchMatrix(matches,children,type){var groups={},keys=[],totalRatio=0,group,order="ordinal-group",BoxOrdinalAttr="data-"+order;matches=filterDuplicates(matches,children,type);forEach(children,function(i,kid){forEach(matches,function(j,x){if(type){group=x[order]||"1";if(x.match===kid){x.match.setAttribute(BoxOrdinalAttr,group);groups[group]=groups[group]||[];groups[group].push(x);}}else{group=x.flex||"0";if(x.match===kid&&(!x[group]||(x[group]&&parseInt(x[group],10)<=1))){totalRatio+=parseInt(group,10);groups[group]=groups[group]||[];groups[group].push(x);}}});if(type&&!kid.getAttribute(BoxOrdinalAttr)){group="1";kid.setAttribute(BoxOrdinalAttr,group);groups[group]=groups[group]||[];groups[group].push({match:kid});}});forEach(groups,function(key){keys.push(key);});keys.sort(function(a,b){return b-a;});return{keys:keys,groups:groups,total:totalRatio};}
...
function cleanPositioningProperties(children){var w,h;forEach(children,function(i,kid){w=kid.style.width;h=kid.style.height;kid.style.cssText=EMPTY_STRING;kid.style.width=w;kid.style.height=h;});}
...
function dimensionValues(target,prop){var parent=target.parentNode,obj,dimension;if(parent.FLX_DOM_ID){obj=FLEX_BOXES[parent.FLX_DOM_ID];forEach(obj.properties,function(i,rule){if((new RegExp(prop)).test(rule.property)){dimension=TRUE;return FALSE;}});}
return dimension;}
function updateChildValues(params){if(params.flexMatrix){forEach(params.children,function(i,x){x.flex=params.flexMatrix[i];});}
if(params.ordinalMatrix){forEach(params.children,function(i,x){x["ordinal-group"]=params.ordinalMatrix[i];});}
return params;}
...
function patchStyleSheet(cssText){return cssText.replace(RE_SELECTOR_GROUP,function(m,prefix,selectorText){var selectorGroups,selector;selectorGroups=selectorText.split(",");forEach(selectorGroups,function(i,group){selector=normalizeSelectorWhitespace(group)+SPACE_STRING;});return prefix+selectorGroups.join(",");});}
...
function loadStyleSheet(url){var xhr=getXHRObject();xhr.open("GET",url,FALSE);xhr.send();return(xhr.status===200)?xhr.responseText:EMPTY_STRING;}
...
return EMPTY_STRING;}
return function(){var url,stylesheet,i,j,baseTags=doc.getElementsByTagName("BASE"),baseUrl=(baseTags.length>0)?baseTags[0].href:doc.location.href,cssText,tree,flexers;for(i=0,j=doc.styleSheets.length;i<j;i++){stylesheet=doc.styleSheets[i];if(stylesheet&&stylesheet.href!==NULL){url=resolveUrl(stylesheet.href,baseUrl);if(url){cssText=patchStyleSheet(parseStyleSheet(url));tree=buildSelectorTree(cssText);flexers=findFlexboxElements(tree);}}}
buildFlexieCall(flexers);};}());FLX.box=function(params){return this.renderModel(params);};FLX.box.prototype={properties:{boxModel:function(target,children,params){var selector,stylesheet,paddingFix,generatedRules;target.style.display="block";if(BROWSER.IE===8){target.style.overflow="hidden";}
if(!params.cleared){selectors=params.selector.split(/\s?,\s?/);stylesheet=doc.styleSheets;stylesheet=stylesheet[stylesheet.length-1];paddingFix="padding-top:"+(getComputedStyle(target,PADDING_TOP,NULL)||"0.1px;");generatedRules=["content: '.'","display: block","height: 0","overflow: hidden"].join(";");forEach(selectors,function(i,selector){if(stylesheet.addRule){if(BROWSER.IE<8){target.style.zoom="1";if(BROWSER.IE===6){stylesheet.addRule(selector.replace(/\>|\+|\~/g,""),paddingFix+"zoom:1;",0);}else if(BROWSER.IE===7){stylesheet.addRule(selector,paddingFix+"display:inline-block;",0);}}else{stylesheet.addRule(selector,paddingFix,0);stylesheet.addRule(selector+":before",generatedRules,0);stylesheet.addRule(selector+":after",generatedRules+";clear:both;",0);}}else if(stylesheet.insertRule){stylesheet.insertRule(selector+"{"+paddingFix+"}",0);stylesheet.insertRule(selector+":after{"+generatedRules+";clear:both;}",0);}});params.cleared=TRUE;}},boxDirection:function(target,children,params){var nestedSelector,nested;if((params.direction==="reverse"&&!params.reversed)||(params.direction==="normal"&&params.reversed)){children=children.reverse();forEach(children,function(i,kid){target.appendChild(kid);});nestedSelector=LIBRARY(params.nested);forEach(nestedSelector,function(i,node){nested=FLEX_BOXES[node.FLX_DOM_ID];if(nested&&nested.direction===INHERIT){nested.direction=params.direction;}});params.reversed=!params.reversed;}},boxOrient:function(target,children,params){var self=this,wide,high;wide={pos:"marginLeft",opp:"marginRight",dim:"width",out:"offsetWidth",func:clientWidth,pad:[PADDING_LEFT,PADDING_RIGHT,BORDER_LEFT,BORDER_RIGHT]};high={pos:"marginTop",opp:"marginBottom",dim:"height",out:"offsetHeight",func:clientHeight,pad:[PADDING_TOP,PADDING_BOTTOM,BORDER_TOP,BORDER_BOTTOM]};if(!SUPPORT){forEach(children,function(i,kid){kid.style[(BROWSER.IE>=9)?"cssFloat":"styleFloat"]=LEFT;if(params.orient===VERTICAL||params.orient===BLOCK_AXIS){kid.style.clear=LEFT;}
...
break;}
target.style.overflow="";}},setup:function(target,children,params){var self=this,matrix,flexCheck;if(!target||!children||!params){return;}
...
self.setup(target,children,params);self.bubbleUp(target,params);},renderModel:function(params){var self=this,target=params.target,nodes=target.childNodes;if(!target.length&&!nodes){return false;}
...
parent=parent.parentNode;}}};FLX.updateInstance=function(target,params){var box;if(target){box=FLEX_BOXES[target.FLX_DOM_ID];if(box){box._instance.updateModel(box);}else{box=new FLX.box(params);}}else{forEach(FLEX_BOXES,function(i,box){box._instance.updateModel(box);});}};FLX.getInstance=function(target){return FLEX_BOXES[target.FLX_DOM_ID];};FLX.destroyInstance=function(target){var box,destroy;destroy=function(box){box.target.FLX_DOM_ID=NULL;box.target.style.cssText=EMPTY_STRING;forEach(box.children,function(i,x){x.match.style.cssText=EMPTY_STRING;});};if(target){box=FLEX_BOXES[target.FLX_DOM_ID];if(box){destroy(box);}}else{forEach(FLEX_BOXES,function(i,box){destroy(box);});FLEX_BOXES=[];}};FLX.flexboxSupport=function(){var partialSupportGrid={},height=100,childHeight,dummy=doc.createElement("flxbox"),child='<b style="margin: 0; padding: 0; display:block; width: 10px; height:'+(height/2)+'px"></b>',tests,result;dummy.style.width=dummy.style.height=height+"px";dummy.innerHTML=(child+child+child);appendProperty(dummy,"display","box",NULL);appendProperty(dummy,"box-align","stretch",TRUE);appendProperty(dummy,"box-pack","justify",TRUE);doc.body.appendChild(dummy);childHeight=dummy.firstChild.offsetHeight;tests={boxAlignStretch:function(){return(childHeight===100);},boxPackJustify:function(){var totalOffset=0;forEach(dummy.childNodes,function(i,child){totalOffset+=child.offsetLeft;});return(totalOffset===135);}};forEach(tests,function(key,value){result=value();if(!result){partialSupportGrid.partialSupport=TRUE;}
partialSupportGrid[key]=result;});doc.body.removeChild(dummy);return~(dummy.style.display).indexOf("box")?partialSupportGrid:FALSE;};FLX.init=function(){FLX.flexboxSupported=SUPPORT=FLX.flexboxSupport();if((!SUPPORT||SUPPORT.partialSupport)&&LIBRARY){selectivizrEngine();}};FLX.version="1.0.3";attachLoadMethod(FLX.init);return FLX;}(this,document));