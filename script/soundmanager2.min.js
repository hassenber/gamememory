...
if(_html5OK(_tO)){oSound=make();_s._wD('Creating sound '+_tO.id+', using HTML5');oSound._setup_html5(_tO);}else{if(_fV>8){if(_tO.isMovieStar===null){_tO.isMovieStar=!!(_tO.serverURL||(_tO.type?_tO.type.match(_netStreamMimeTypes):false)||_tO.url.match(_netStreamPattern));}
...
return _s.sounds[sID].clearOnPosition(nPosition,oMethod);};this.play=function(sID,oOptions){var result=false;if(!_didInit||!_s.ok()){_complain(_sm+'.play(): '+_str(!_didInit?'notReady':'notOK'));return result;}
...
return _s.sounds[sID].pause();};this.pauseAll=function(){var i;for(i=_s.soundIDs.length-1;i>=0;i--){_s.sounds[_s.soundIDs[i]].pause();}};this.resume=function(sID){if(!_idCheck(sID)){return false;}
return _s.sounds[sID].resume();};this.resumeAll=function(){var i;for(i=_s.soundIDs.length-1;i>=0;i--){_s.sounds[_s.soundIDs[i]].resume();}};this.togglePause=function(sID){if(!_idCheck(sID)){return false;}
...
_disabled=true;_wDS('shutdown',1);for(i=_s.soundIDs.length-1;i>=0;i--){_disableObject(_s.sounds[_s.soundIDs[i]]);}
...
return result;};this.canPlayLink=function(oLink){if(typeof oLink.type!=='undefined'&&oLink.type){if(_s.canPlayMIME(oLink.type)){return true;}}
...
return result;};this.onready=function(oMethod,oScope){var sType='onready',result=false;if(typeof oMethod==='function'){if(_didInit){_s._wD(_str('queue',sType));}
if(!oScope){oScope=_win;}
_addOnEvent(sType,oMethod,oScope);_processOnEvents();result=true;}else{throw _str('needFunction',sType);}
return result;};this.ontimeout=function(oMethod,oScope){var sType='ontimeout',result=false;if(typeof oMethod==='function'){if(_didInit){_s._wD(_str('queue',sType));}
if(!oScope){oScope=_win;}
_addOnEvent(sType,oMethod,oScope);_processOnEvents({type:sType});result=true;}else{throw _str('needFunction',sType);}
return result;};this._writeDebug=function(sText,sType,_bTimestamp){var sDID='soundmanager-debug',o,oItem,sMethod;if(!_s.debugMode){return false;}
...
_oRemovedHTML=_oRemoved=_needsFlash=null;_s.enabled=_didDCLoaded=_didInit=_waitingForEI=_initPending=_didAppend=_appendSuccess=_disabled=_s.swfLoaded=false;_s.soundIDs=[];_s.sounds={};_flash=null;for(i in _on_queue){if(_on_queue.hasOwnProperty(i)){for(j=_on_queue[i].length-1;j>=0;j--){_on_queue[i][j].fired=false;}}}
...
return _t;}
_iO=_t._iO;_lastURL=_t.url;_t.loaded=false;_t.readyState=1;_t.playState=0;_t.id3={};if(_html5OK(_iO)){oS=_t._setup_html5(_iO);if(!oS._called_load){_s._wD(_h5+'load: '+_t.id);_t._html5_canplay=false;if(_t._a.src!==_iO.url){_s._wD(_wDS('manURL')+': '+_iO.url);_t._a.src=_iO.url;_t.setPosition(0);}
_t._a.autobuffer='auto';_t._a.preload='auto';oS._called_load=true;if(_iO.autoPlay){_t.play();}}else{_s._wD(_h5+'ignoring request to load again: '+_t.id);}}else{try{_t.isHTML5=false;_t._iO=_policyFix(_loopFix(_iO));_iO=_t._iO;if(_fV===8){_flash._load(_t.id,_iO.url,_iO.stream,_iO.autoPlay,(_iO.whileloading?1:0),_iO.loops||1,_iO.usePolicyFile);}else{_flash._load(_t.id,_iO.url,!!(_iO.stream),!!(_iO.autoPlay),_iO.loops||1,!!(_iO.autoLoad),_iO.usePolicyFile);}}catch(e){_wDS('smError',2);_debugTS('onload',false);_catchError({type:'SMSOUND_LOAD_JS_EXCEPTION',fatal:true});}}
return _t;};this.unload=function(){if(_t.readyState!==0){_s._wD('SMSound.unload(): "'+_t.id+'"');if(!_t.isHTML5){if(_fV===8){_flash._unload(_t.id,_emptyURL);}else{_flash._unload(_t.id);}}else{_stop_html5_timer();if(_t._a){_t._a.pause();_html5Unload(_t._a,_emptyURL);_t.url=_emptyURL;}}
_resetProperties();}
...
if(!_bFromSM){_s.destroySound(_t.id,true);}};this.play=function(oOptions,_updatePlayState){var fN,allowMulti,a,onready,startOK=true,exit=null;fN='SMSound.play(): ';_updatePlayState=(typeof _updatePlayState==='undefined'?true:_updatePlayState);if(!oOptions){oOptions={};}
...
if(!_t.loaded){if(_t.readyState===0){_s._wD(fN+'Attempting to load "'+_t.id+'"',1);if(!_t.isHTML5){_t._iO.autoPlay=true;_t.load(_t._iO);}else{_t.load(_t._iO);}}else if(_t.readyState===2){_s._wD(fN+'Could not load "'+_t.id+'" - exiting',2);exit=_t;}else{_s._wD(fN+'"'+_t.id+'" is loading - attempting to play..',1);}}else{_s._wD(fN+'"'+_t.id+'"');}
if(exit!==null){return exit;}
...
if(_t.paused&&_t.position&&_t.position>0){_s._wD(fN+'"'+_t.id+'" is resuming from paused state',1);_t.resume();}else{_t._iO=_mixin(oOptions,_t._iO);if(_t._iO.from!==null&&_t._iO.to!==null&&_t.instanceCount===0&&_t.playState===0&&!_t._iO.serverURL){onready=function(){_t._iO=_mixin(oOptions,_t._iO);_t.play(_t._iO);};if(_t.isHTML5&&!_t._html5_canplay){_s._wD(fN+'Beginning load of "'+_t.id+'" for from/to case');_t.load({_oncanplay:onready});exit=false;}else if(!_t.isHTML5&&!_t.loaded&&(!_t.readyState||_t.readyState!==2)){_s._wD(fN+'Preloading "'+_t.id+'" for from/to case');_t.load({onload:onready});exit=false;}
if(exit!==null){return exit;}
_t._iO=_applyFromTo();}
...
_t.setVolume(_t._iO.volume,true);_t.setPan(_t._iO.pan,true);if(!_t.isHTML5){startOK=_flash._start(_t.id,_t._iO.loops||1,(_fV===9?_t._iO.position:_t._iO.position/1000),_t._iO.multiShot);if(_fV===9&&!startOK){_s._wD(fN+_t.id+': No sound hardware, or 32-sound ceiling hit');if(_t._iO.onplayerror){_t._iO.onplayerror.apply(_t);}}}else{_start_html5_timer();a=_t._setup_html5();_t.setPosition(_t._iO.position);a.play();}}
...
_s._wD('SMSound.pause()');_t.paused=true;if(!_t.isHTML5){if(_bCallFlash||typeof _bCallFlash==='undefined'){_flash._pause(_t.id,_t._iO.multiShot);}}else{_t._setup_html5().pause();_stop_html5_timer();}
...
return _t;};this.mute=function(){_t.muted=true;if(!_t.isHTML5){_flash._setVolume(_t.id,0);}else if(_t._a){_t._a.muted=true;}
return _t;};this.unmute=function(){_t.muted=false;var hasIO=(typeof _t._iO.volume!=='undefined');if(!_t.isHTML5){_flash._setVolume(_t.id,hasIO?_t._iO.volume:_t.options.volume);}else if(_t._a){_t._a.muted=false;}
...
for(i=j-1;i>=0;i--){item=_onPositionItems[i];if(!item.fired&&_t.position>=item.position){item.fired=true;_onPositionFired++;item.method.apply(item.scope,[item.position]);}}
return true;};this._resetOnPosition=function(nPosition){var i,item,j=_onPositionItems.length;if(!j){return false;}
for(i=j-1;i>=0;i--){item=_onPositionItems[i];if(item.fired&&nPosition<=item.position){item.fired=false;_onPositionFired--;}}
return true;};_applyFromTo=function(){var _iO=_t._iO,f=_iO.from,t=_iO.to,start,end;end=function(){_s._wD(_t.id+': "to" time of '+t+' reached.');_t.clearOnPosition(t,end);_t.stop();};start=function(){_s._wD(_t.id+': playing "from" '+f);if(t!==null&&!isNaN(t)){_t.onPosition(t,end);}};if(f!==null&&!isNaN(f)){_iO.position=f;_iO.multiShot=false;start();}
...
return isNew;}};this._get_html5_duration=function(){var _iO=_t._iO,d=(_t._a?_t._a.duration*1000:(_iO?_iO.duration:undefined)),result=(d&&!isNaN(d)&&d!==Infinity?d:(_iO?_iO.duration:null));return result;};this._apply_loop=function(a,nLoops){if(!a.loop&&nLoops>1){_s._wD('Note: Native HTML5 looping is infinite.');}
...
_resetProperties((_oldIO&&_oldIO.url?_iO.url===_oldIO.url:(_lastURL?_lastURL===_iO.url:false)));_a.src=_iO.url;_t.url=_iO.url;_lastURL=_iO.url;_a._called_load=false;}else{_wDS('h5a');if(_iO.autoLoad||_iO.autoPlay){_t._a=new Audio(_iO.url);}else{_t._a=(_isOpera?new Audio(null):new Audio());}
_a=_t._a;_a._called_load=false;if(_useGlobalHTML5Audio){_s._global_a=_a;}}
_t.isHTML5=true;_t._a=_a;_a._t=_t;_add_html5_events();_t._apply_loop(_a,_iO.loops);if(_iO.autoLoad||_iO.autoPlay){_t.load();}else{_a.autobuffer=false;_a.preload='auto';}
return _a;};_add_html5_events=function(){if(_t._a._added_events){return false;}
var f;function add(oEvt,oFn,bCapture){return _t._a?_t._a.addEventListener(oEvt,oFn,bCapture||false):null;}
_t._a._added_events=true;for(f in _html5_events){if(_html5_events.hasOwnProperty(f)){add(f,_html5_events[f]);}}
...
_s._wD(_h5+'removing event listeners: '+_t.id);_t._a._added_events=false;for(f in _html5_events){if(_html5_events.hasOwnProperty(f)){remove(f,_html5_events[f]);}}};this._onload=function(nSuccess){var fN,loadOK=(!!(nSuccess)||(!_t.isHTML5&&_fV===8&&_t.duration));fN='SMSound._onload(): ';_s._wD(fN+'"'+_t.id+'"'+(loadOK?' loaded.':' failed to load? - '+_t.url),(loadOK?1:2));if(!loadOK&&!_t.isHTML5){if(_s.sandbox.noRemote===true){_s._wD(fN+_str('noNet'),1);}
...
if(typeof _t.durationEstimate==='undefined'){_t.durationEstimate=_t.duration;}}else{_t.durationEstimate=_t.duration;}
if(!_t.isHTML5){_t.buffered=[{'start':0,'end':_t.duration}];}
if((_t.readyState!==3||_t.isHTML5)&&_iO.whileloading){_iO.whileloading.apply(_t);}};this._whileplaying=function(nPosition,oPeakData,oWaveformDataLeft,oWaveformDataRight,oEQData){var _iO=_t._iO,eqLeft;if(isNaN(nPosition)||nPosition===null){return false;}
...
return true;};this._oncaptiondata=function(oData){_s._wD('SMSound._oncaptiondata(): "'+this.id+'" caption data received.');_t.captiondata=oData;if(_t._iO.oncaptiondata){_t._iO.oncaptiondata.apply(_t);}};this._onmetadata=function(oMDProps,oMDData){_s._wD('SMSound._onmetadata(): "'+this.id+'" metadata received.');var oData={},i,j;for(i=0,j=oMDProps.length;i<j;i++){oData[oMDProps[i]]=oMDData[i];}
...
return o1;};_extraOptions={'onready':1,'ontimeout':1,'defaultOptions':1,'flash9Options':1,'movieStarOptions':1};_assign=function(o,oParent){var i,result=true,hasParent=(typeof oParent!=='undefined'),setupOptions=_s.setupOptions,extraOptions=_extraOptions;if(typeof o==='undefined'){result=[];for(i in setupOptions){if(setupOptions.hasOwnProperty(i)){result.push(i);}}
...
_s._wD(_str('setup',result.join(', ')));return false;}
for(i in o){if(o.hasOwnProperty(i)){if(typeof o[i]!=='object'||o[i]===null||o[i]instanceof Array){if(hasParent&&typeof extraOptions[oParent]!=='undefined'){_s[oParent][i]=o[i];}else if(typeof setupOptions[i]!=='undefined'){_s.setupOptions[i]=o[i];_s[i]=o[i];}else if(typeof extraOptions[i]==='undefined'){_complain(_str((typeof _s[i]==='undefined'?'setupUndef':'setupError'),i),2);result=false;}else{if(_s[i]instanceof Function){_s[i].apply(_s,(o[i]instanceof Array?o[i]:[o[i]]));}else{_s[i]=o[i];}}}else{if(typeof extraOptions[i]==='undefined'){_complain(_str((typeof _s[i]==='undefined'?'setupUndef':'setupError'),i),2);result=false;}else{return _assign(o[i],i);}}}}
return result;};_event=(function(){var old=(_win.attachEvent),evt={add:(old?'attachEvent':'addEventListener'),remove:(old?'detachEvent':'removeEventListener')};function getArgs(oArgs){var args=_slice.call(oArgs),len=args.length;if(old){args[1]='on'+args[1];if(len>3){args.pop();}}else if(len===3){args.push(false);}
return args;}
...
return{'add':add,'remove':remove};}());function _preferFlashCheck(kind){return(_s.preferFlash&&_hasFlash&&!_s.ignoreFlash&&(typeof _s.flash[kind]!=='undefined'&&_s.flash[kind]));}
function _html5_event(oFn){return function(e){var t=this._t,result;if(!t||!t._a){if(t&&t.id){_s._wD(_h5+'ignoring '+e.type+': '+t.id);}else{_s._wD(_h5+'ignoring '+e.type);}
result=null;}else{result=oFn.call(this,e);}
return result;};}
_html5_events={abort:_html5_event(function(){_s._wD(_h5+'abort: '+this._t.id);}),canplay:_html5_event(function(){var t=this._t,position1K;if(t._html5_canplay){return true;}
t._html5_canplay=true;_s._wD(_h5+'canplay: '+t.id+', '+t.url);t._onbufferchange(0);position1K=(typeof t._iO.position!=='undefined'&&!isNaN(t._iO.position)?t._iO.position/1000:null);if(t.position&&this.currentTime!==position1K){_s._wD(_h5+'canplay: setting position to '+position1K);try{this.currentTime=position1K;}catch(ee){_s._wD(_h5+'setting position of '+position1K+' failed: '+ee.message,2);}}
if(t._iO._oncanplay){t._iO._oncanplay();}}),canplaythrough:_html5_event(function(){var t=this._t;if(!t.loaded){t._onbufferchange(0);t._whileloading(t.bytesLoaded,t.bytesTotal,t._get_html5_duration());t._onload(true);}}),ended:_html5_event(function(){var t=this._t;_s._wD(_h5+'ended: '+t.id);t._onfinish();}),error:_html5_event(function(){_s._wD(_h5+'error: '+this.error.code);this._t._onload(false);}),loadeddata:_html5_event(function(){var t=this._t;_s._wD(_h5+'loadeddata: '+this._t.id);if(!t._loaded&&!_isSafari){t.duration=t._get_html5_duration();}}),loadedmetadata:_html5_event(function(){_s._wD(_h5+'loadedmetadata: '+this._t.id);}),loadstart:_html5_event(function(){_s._wD(_h5+'loadstart: '+this._t.id);this._t._onbufferchange(1);}),play:_html5_event(function(){_s._wD(_h5+'play: '+this._t.id+', '+this._t.url);this._t._onbufferchange(0);}),playing:_html5_event(function(){_s._wD(_h5+'playing: '+this._t.id);this._t._onbufferchange(0);}),progress:_html5_event(function(e){var t=this._t,i,j,str,buffered=0,isProgress=(e.type==='progress'),ranges=e.target.buffered,loaded=(e.loaded||0),total=(e.total||1);t.buffered=[];if(ranges&&ranges.length){for(i=0,j=ranges.length;i<j;i++){t.buffered.push({'start':ranges.start(i),'end':ranges.end(i)});}
buffered=(ranges.end(0)-ranges.start(0));loaded=buffered/e.target.duration;if(isProgress&&ranges.length>1){str=[];j=ranges.length;for(i=0;i<j;i++){str.push(e.target.buffered.start(i)+'-'+e.target.buffered.end(i));}
_s._wD(_h5+'progress: timeRanges: '+str.join(', '));}
if(isProgress&&!isNaN(loaded)){_s._wD(_h5+'progress: '+t.id+': '+Math.floor(loaded*100)+'% loaded');}}
if(!isNaN(loaded)){t._onbufferchange(0);t._whileloading(loaded,total,t._get_html5_duration());if(loaded&&total&&loaded===total){_html5_events.canplaythrough.call(this,e);}}}),ratechange:_html5_event(function(){_s._wD(_h5+'ratechange: '+this._t.id);}),suspend:_html5_event(function(e){var t=this._t;_s._wD(_h5+'suspend: '+t.id);_html5_events.progress.call(this,e);t._onsuspend();}),stalled:_html5_event(function(){_s._wD(_h5+'stalled: '+this._t.id);}),timeupdate:_html5_event(function(){this._t._onTimer();}),waiting:_html5_event(function(){var t=this._t;_s._wD(_h5+'waiting: '+t.id);t._onbufferchange(1);})};_html5OK=function(iO){var result;if(iO.serverURL||(iO.type&&_preferFlashCheck(iO.type))){result=false;}else{result=((iO.type?_html5CanPlay({type:iO.type}):_html5CanPlay({url:iO.url})||_s.html5Only));}
return result;};_html5Unload=function(oAudio,url){if(oAudio){oAudio.src=url;}};_html5CanPlay=function(o){if(!_s.useHTML5Audio||!_s.hasHTML5){return false;}
...
fileExt=(url?url.toLowerCase().match(_html5Ext):null);if(!fileExt||!fileExt.length){if(!mime){result=false;}else{offset=mime.indexOf(';');fileExt=(offset!==-1?mime.substr(0,offset):mime).substr(6);}}else{fileExt=fileExt[1];}
...
return result;};_testHTML5=function(){if(!_s.useHTML5Audio||typeof Audio==='undefined'){return false;}
var a=(typeof Audio!=='undefined'?(_isOpera?new Audio(null):new Audio()):null),item,lookup,support={},aF,i;function _cp(m){var canPlay,i,j,result=false,isOK=false;if(!a||typeof a.canPlayType!=='function'){return result;}
if(m instanceof Array){for(i=0,j=m.length;i<j&&!isOK;i++){if(_s.html5[m[i]]||a.canPlayType(m[i]).match(_s.html5Test)){isOK=true;_s.html5[m[i]]=true;_s.flash[m[i]]=!!(m[i].match(_flashMIME));}}
result=isOK;}else{canPlay=(a&&typeof a.canPlayType==='function'?a.canPlayType(m):false);result=!!(canPlay&&(canPlay.match(_s.html5Test)));}
return result;}
aF=_s.audioFormats;for(item in aF){if(aF.hasOwnProperty(item)){lookup='audio/'+item;support[item]=_cp(aF[item].type);support[lookup]=support[item];if(item.match(_flashMIME)){_s.flash[item]=true;_s.flash[lookup]=true;}else{_s.flash[item]=false;_s.flash[lookup]=false;}
...
_on_queue[sType].push({'method':oMethod,'scope':(oScope||null),'fired':false});};_processOnEvents=function(oOptions){if(!oOptions){oOptions={type:(_s.ok()?'onready':'ontimeout')};}
if(!_didInit&&oOptions&&!oOptions.ignoreInit){return false;}
if(oOptions.type==='ontimeout'&&(_s.ok()||(_disabled&&!oOptions.ignoreInit))){return false;}
...
hasPlugin=(!!obj);}
_hasFlash=hasPlugin;return hasPlugin;};_featureCheck=function(){var needsFlash,item,result=true,formats=_s.audioFormats,isSpecial=(_is_iDevice&&!!(_ua.match(/os (1|2|3_0|3_1)/i)));if(isSpecial){_s.hasHTML5=false;_s.html5Only=true;if(_s.oMC){_s.oMC.style.display='none';}
result=false;}else{if(_s.useHTML5Audio){if(!_s.html5||!_s.html5.canPlayType){_s._wD('SoundManager: No HTML5 Audio() support detected.');_s.hasHTML5=false;}else{_s.hasHTML5=true;}
...
if(_s.useHTML5Audio&&_s.hasHTML5){for(item in formats){if(formats.hasOwnProperty(item)){if((formats[item].required&&!_s.html5.canPlayType(formats[item].type))||(_s.preferFlash&&(_s.flash[item]||_s.flash[formats[item].type]))){needsFlash=true;}}}}
...
return result;};_startTimer=function(oSound){if(!oSound._hasTimer){oSound._hasTimer=true;if(!_mobileHTML5&&_s.html5PollingInterval){if(_h5IntervalTimer===null&&_h5TimerCount===0){_h5IntervalTimer=_win.setInterval(_timerExecute,_s.html5PollingInterval);}
_h5TimerCount++;}}};_stopTimer=function(oSound){if(oSound._hasTimer){oSound._hasTimer=false;if(!_mobileHTML5&&_s.html5PollingInterval){_h5TimerCount--;}}};_timerExecute=function(){var i;if(_h5IntervalTimer!==null&&!_h5TimerCount){_win.clearInterval(_h5IntervalTimer);_h5IntervalTimer=null;return false;}
...
setTimeout(_init,_isIE?100:1);};_createMovie=function(smID,smURL){if(_didAppend&&_appendSuccess){return false;}
...
return true;};_delayWaitForEI=function(){setTimeout(_waitForEI,1000);};_waitForEI=function(){var p,loadIncomplete=false;if(_waitingForEI){return false;}
_waitingForEI=true;_event.remove(_win,'load',_delayWaitForEI);if(_tryInitOnFocus&&!_isFocused){_wDS('waitFocus');return false;}
if(!_didInit){p=_s.getMoviePercent();_s._wD(_str('waitImpatient',(p>0?' (SWF '+p+'% loaded)':'')));if(p>0&&p<100){loadIncomplete=true;}}
setTimeout(function(){p=_s.getMoviePercent();if(loadIncomplete){_waitingForEI=false;_s._wD(_str('waitSWF'));_win.setTimeout(_delayWaitForEI,1);return false;}
...
_wDS('waitForever');}else{_failSafely(true);}}else{if(_s.flashLoadTimeout===0){_wDS('waitForever');}else{_failSafely(true);}}}},_s.flashLoadTimeout);};_handleFocus=function(){function cleanup(){_event.remove(_win,'focus',_handleFocus);}
if(_isFocused||!_tryInitOnFocus){cleanup();return true;}
_okToDisable=true;_isFocused=true;_s._wD(_sm+': Got window focus.');_waitingForEI=false;_delayWaitForEI();cleanup();return true;};_showSupport=function(){var item,tests=[];if(_s.useHTML5Audio&&_s.hasHTML5){for(item in _s.audioFormats){if(_s.audioFormats.hasOwnProperty(item)){tests.push(item+': '+_s.html5[item]+(!_s.html5[item]&&_hasFlash&&_s.flash[item]?' (using flash)':(_s.preferFlash&&_s.flash[item]&&_hasFlash?' (preferring flash)':(!_s.html5[item]?' ('+(_s.audioFormats[item].required?'required, ':'')+'and no flash support)':''))));}}
...
var wasTimeout=(_s.useFlashBlock&&_s.flashLoadTimeout&&!_s.getMoviePercent()),result=true,error;if(!wasTimeout){_didInit=true;if(_disabled){error={type:(!_hasFlash&&_needsFlash?'NO_FLASH':'INIT_TIMEOUT')};}}
...
_processOnEvents({type:'ontimeout',error:error,ignoreInit:true});_debugTS('onload',false);_catchError(error);result=false;}else{_debugTS('onload',true);}
...
_initUserOnload();}}
return result;};_setProperties=function(){var i,o=_s.setupOptions;for(i in o){if(o.hasOwnProperty(i)){if(typeof _s[i]==='undefined'){_s[i]=o[i];}else if(_s[i]!==o[i]){_s.setupOptions[i]=_s[i];}}}};_init=function(){_wDS('init');if(_didInit){_wDS('didInit');return false;}
...
_didDCLoaded=true;_setProperties();_initDebug();(function(){var a='sm2-usehtml5audio=',a2='sm2-preferflash=',b=null,b2=null,hasCon=(typeof console!=='undefined'&&typeof console.log==='function'),l=_wl.toLowerCase();if(l.indexOf(a)!==-1){b=(l.charAt(l.indexOf(a)+a.length)==='1');if(hasCon){console.log((b?'Enabling ':'Disabling ')+'useHTML5Audio via URL parameter');}
...
_s.setup({'preferFlash':b2});}}());if(!_hasFlash&&_s.hasHTML5){_s._wD('SoundManager: No Flash detected'+(!_s.useHTML5Audio?', enabling HTML5.':'. Trying HTML5-only mode.'));_s.setup({'useHTML5Audio':true,'preferFlash':false});}
_testHTML5();_s.html5.usingFlash=_featureCheck();_needsFlash=_s.html5.usingFlash;_showSupport();if(!_hasFlash&&_needsFlash){_s._wD('SoundManager: Fatal error: Flash is needed to play some required formats, but is not available.');_s.setup({'flashLoadTimeout':1});}
